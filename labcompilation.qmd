...
Now we try two candidate models `M1` @eq-linearm1 and `M2` @eq-linearm2:

$$M1: y =\beta_0 + \beta_1 x_1 + \beta_2 x_2 + \beta_3 x_3 + \beta_4 x_4 + \epsilon$$ 

$$M2: y = \beta_0 + \beta_1 x_3 + \beta_2 x_4 + \beta_3(x_3 \times x_4) + \epsilon$$  

We use re-sampling methods to compare their performance.
...
Finally, calculate the cross-validation MSE for each model by obtaining the mean of each CV model. 
